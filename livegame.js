...
const getParticipantsData = async (kayn, participants) => {
  ...
  for await (const pl of participants) {
    try {
      ...
      console.log(summoner.accountId, pl.teamId, list.length, champions.data[pl.championId].name);
...
const main = async () => {
  try {
    ...
    const modelData = await ClassificationModel.findOne()
    ...
    const model = await tf.loadLayersModel(
      tf.io.fromMemory(
        ...
        new Uint8Array(modelData.weightData.buffer).buffer,
      ),
    );
    ...
    const testingData = tf.tensor3d([
      [sumsBlue, sumsRed].map((player) => {
        return player.map((stat, index) => {
